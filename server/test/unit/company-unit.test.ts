import { Pos3DZero, RESOURCE_TYPE } from "@logisim/lib/entities";
import { describe, it, expect } from "vitest";
import { createWorld } from "../../src/world/world";
import { Color } from "@logisim/lib/utils";
import { collectFromCompany } from "../../src/world/companies";
import {
  breakContract,
...
describe("collectFromCompany unit tests", () => {
  ...
  const creditorCompany = world.createCompany("Creditor Co", 100000, Color.Red);
  ...
  const debtorCompany = world.createCompany("Debtor Inc", 100000, Color.Blue);
  ...
  */
  it("should recover the full amount from the debtor company, because they can afford it", () => {
    ...
    expect(debtorCompany.money === 0);
    expect(debtorCompany.isInsolvent === true);
    expect(creditorCompany.money === 200001);
  });
