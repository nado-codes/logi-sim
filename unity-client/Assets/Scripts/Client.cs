    ...
    private void RefreshActiveCompanyContractStates()
    {
        var expiredContracts = ContractDTOs.Where(c => c.ExpectedTick <= WorldTick).ToList();
            ...
            foreach(var contract in companyContacts)
            {
                ...
                PopupController.ShowPrompt("Contract Expired", $"Contract {contract.Id} has expired at tick {WorldTick}. You now owe {contractCompany?.Name} money for the failed contract. Due to the debt, your company is now insolvent and you cannot take new contracts or purchase assets until you resolve your debts.");
